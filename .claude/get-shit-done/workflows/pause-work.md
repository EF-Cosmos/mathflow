...
```bash
timestamp=$(node "E:/project/laptop/classroom/linux-homework/mathflow/.claude/get-shit-done/bin/gsd-tools.cjs" current-timestamp full --raw)
```
...
<step name="commit">
```bash
node "E:/project/laptop/classroom/linux-homework/mathflow/.claude/get-shit-done/bin/gsd-tools.cjs" commit "wip: [context-name] paused at [X]/[Y]" --files [handoff-path] .planning/HANDOFF.json
```
</step>
